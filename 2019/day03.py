class Wire():
    def __init__(self, segments):
        ...
        for segment in segments:
            self.wire_segments.append(WireSegment(segment))
        self.coords = []
        x = 0
        y = 0
        total_steps = 1
...
class WireSegment():
    def __init__(self, segment_string):
        self.direction = segment_string[0]
...
def get_closest_intersection(wire1_coords, wire2_coords):
    wire1 = Wire(wire1_coords)
    wire2 = Wire(wire2_coords)

    intersections = list(set(wire1.coords).intersection(set(wire2.coords)))
    intersections.sort(key=lambda x: x.distance_from_origin())
    distance = intersections[0].distance_from_origin()
    return distance

assert get_closest_intersection(
